...
$id = intval($_GET['id']); // can be forged.
$code = sql($_GET['code']);
...
if (mysqli_num_rows($result) == 0) {
	...
	$output = '';
	$result = mysqli_query($link, "SELECT id FROM hacker WHERE chat_mention = 1 AND id = $id"); // did you get mentioned?
	$output = mysqli_num_rows($result); // this should be 1 if mentioned, 0 if not
	echo $output;
	@mysqli_free_result($result);
	@mysqli_close($link);
}
