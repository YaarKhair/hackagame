...
<div id="profile-wrapper">
	...
	<div id="profile-info" class="hr-light">
		<div class="row mv10">
			<div class="col w25"><?php echo PrintIcon('level', 'large').GetHackerLevel($row['id']); ?></div>
		...
		<div class="row mv10">
			<?php
				...
				if(!InGroup($row['id'], 1) && !InGroup($row['id'], 2)) {
					...
					$worldrank = number_format($worldranklink);
				}
			?>
			<div class="col w25"><?php echo '<a href="?h=worldrank#'.$worldranklink.'">'.PrintIcon('rank', 'large').'#'.$worldrank.'</a>'; ?></div>
		...
		<div class="row mv10">
			<div class="col w25"><?php echo PrintIcon('hackpoints', 'large').$row['hackpoints']; ?></div>
			<div class="col w25"><?php echo PrintIcon('server', 'large').NumServers($row['id']).'/'.MaxServers($row['id']); ?></div>
			<div class="col w50"><?php echo PrintIcon('last-seen', 'large').Number2Date($row['last_click']); ?></div>
		</div>
	</div>
	<?php if(strlen($row['prev_alias']) > 0) { ?>
	<p class="mv10 italic">&mdash; also known as "<?php echo $row['prev_alias']; ?>"</p>
	<?php } ?>
</div>
<?php
