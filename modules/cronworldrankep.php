...
<?php	
	$result = mysqli_query($link, "SELECT id, country, started FROM hacker WHERE active = 1 AND npc = 0 AND banned_date = 0 AND alias <> '' AND network_id = 2 AND clan_id <> $staff_clanid ORDER BY ep DESC, skill DESC");
	$number = mysqli_num_rows($result);
	$output = '
			<h1>World Rank List - Sorted by EP and SKill</h1>
				<div class="row th">
					<div class="col w10">#</div>
					<div class="col w30">Alias</div>
					<div class="col w20">Level</div>
	...
	if (mysqli_num_rows($result) > 0) {		
		...
		while ($row = mysqli_fetch_assoc($result)) {
			if (!InStaff($row['id'])) {
				...
				$output .= '<div class="col w20">'.EP2Level(GetHackerEP($row['id'])).'</a></div>';
	...
	$myFile = "$gamepath/pages/worldrankep.php";
