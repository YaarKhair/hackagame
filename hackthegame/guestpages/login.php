...
<?php	
	if (!empty($_POST['email'])) {
		...
		$query = "SELECT ep, email, salt, activationcode, id, banned_date, banned_reason, password, alias, hybernate_till, failed_logins, lockout_till, lockout_reason, clan_id, real_ip FROM `hacker` WHERE `email` = '$email'";
		...
		if (mysqli_num_rows($result) == 0) { 
			...
			if ($row['lockout_till'] > $now && $row['id'] != 8157) 
